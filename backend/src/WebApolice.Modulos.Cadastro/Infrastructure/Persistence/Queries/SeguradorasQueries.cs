...
public class SeguradorasQueries : ISeguradorasQueries
{
    private readonly CadastroDbContext _dbContext;

    public SeguradorasQueries(CadastroDbContext dbContext)
    {
        _dbContext = dbContext;
    }

    public async Task<(IEnumerable<SeguradoraListagemItemResult> itens, int totalItens, int totalPaginas)> ListarPaginadoAsync(
        int pagina, 
        int tamanhoPagina, 
        string? busca, 
        bool? ativo, 
    ...
    {
        var query = from s in _dbContext.Seguradoras.AsNoTracking()
        ...
        if (!string.IsNullOrWhiteSpace(busca))
        {
            ...
            query = query.Where(x => 
                ...
                (x.s.Susep != null && x.s.Susep.ToLower().Contains(buscaLower))
            );
        }
    ...
    public async Task<SeguradoraDetalheResult?> ObterPorPublicIdAsync(Guid publicId, CancellationToken cancellationToken)
    {
        return await (from s in _dbContext.Seguradoras.AsNoTracking()
                      join p in _dbContext.Pessoas.AsNoTracking() on s.PessoaId equals p.Id
                      where s.PublicId == publicId && s.DeletedAt == null
                      select new SeguradoraDetalheResult
                      {
                          PublicId = s.PublicId,
                          Nome = p.Nome,
                          Codigo = s.Codigo,
