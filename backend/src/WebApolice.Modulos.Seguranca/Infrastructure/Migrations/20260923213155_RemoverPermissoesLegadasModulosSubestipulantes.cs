...
{
    /// <inheritdoc />
    public partial class RemoverPermissoesLegadasModulosSubestipulantes : Migration
    {
        /// <inheritdoc />
...
WHERE permissao_id IN (
    SELECT id FROM seguranca.permissao 
    WHERE codigo IN (
        'apolices.subestipulantes.modulos.visualizar',
...
DELETE FROM seguranca.permissao 
WHERE codigo IN (
    'apolices.subestipulantes.modulos.visualizar',
    'apolices.subestipulantes.modulos.inserir',
    'apolices.subestipulantes.modulos.alterar',
    'apolices.subestipulantes.modulos.inativar'
        ...
        {
            // Nota: Os vínculos originais de perfil_permissao são irreversíveis nesta migration.
            migrationBuilder.Sql(@"
DO $$
DECLARE
    v_recurso_id bigint;
BEGIN
    ...
    IF v_recurso_id IS NOT NULL THEN
        ...
        VALUES 
            (gen_random_uuid(), v_recurso_id, 'Visualizar Módulos do Subestipulante', 'apolices.subestipulantes.modulos.visualizar', true),
